...
Behaviour-Driven Development (BDD) served as the core foundation of our software engineering process, providing a shared language that connected the SPIO stakeholders' business needs with our technical work. Rather than treating BDD simply as a way to write tests, we used it as our main design approach to guarantee clear tracking from the initial requirements gathering all the way through to deployment.

\textbf{From Elicitation to Specification:} 
Following our initial client meetings, we structured our workflow to translate broad requirements (e.g., "compare university courses") into clear, testable specifications. We applied the MoSCoW method for prioritisation and the INVEST checklist to validate our User Stories. These stories were then expanded into precise Gherkin scenarios (Given-When-Then). As documented in our initial report, this step was vital for removing confusion. By writing concrete examples of how the system should behave, we prevented scope creep and ensured both the client and the developers shared the exact same definition of "done."
...
\begin{itemize}
    ...
    \item The \texttt{Then} outcomes defined the exact HTTP responses and the resulting React UI updates (e.g., triggering a polling mechanism to display the scraping progress).
\end{itemize}

\textbf{Defining Complex UI and Background Processes:} 
BDD was particularly helpful for defining the User Experience around long background tasks. Because the actual web scraping happens invisibly on the server, we deliberately wrote our scenarios to describe only what the user can see on the screen, rather than the internal scraping code. Scenarios like \texttt{BR 2.2: Attempting to start a fetch while one is already in progress} forced us to build reliable frontend state management (like disabling buttons and showing specific error messages) and backend database checks (using the \texttt{ScrapeStatus} enum) to deliver the expected behaviour.

\textbf{Test Oracles and Validation:} 
Finally, these behavioural specifications served as our absolute Test Oracles for both System Testing and User Acceptance Testing. If a specific piece of code—such as the \texttt{Excel.api.ts} service or the \texttt{EdinburghAdapter}—failed to meet the exact \texttt{Then} conditions outlined in the Requirements Document, that sprint's work was not accepted. This strict use of BDD ensured that every line of code written contributed directly to a confirmed client requirement.
