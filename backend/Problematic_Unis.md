...
University of Aberdeen: Most should work with PDF parsing, or are filled by UCAS, script only ran on online courses which need different parsing logic (keyword differences). Most fees present in DB.

University of Bath: Nuanced course options are not directly linked by the url provided by UCAS. Although they can be found from the UCAS URL most of them do not have tuition fees anyways. Further testing needed, no fees present.

University of Birmingham: Intl fees not properly located due to unique structure (dropdown menu for each country, although all Intl fees are the same). Most fees not present.
