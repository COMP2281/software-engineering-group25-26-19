...
export class ExeterAdapter extends GenericHtmlAdapter {
    ...
    protected override async parseHtml(html: string): Promise<ScrapedFees> {
        if (DEBUG) console.log(`[DEBUG] Exeter: Sanitizing trap keywords and edge-case fees from HTML...`);
        ...
        let cleanedHtml = html
            ...
            // 2. Erase part-time, placement, and year in industry fees
            .replace(/part-?time[^£]{0,80}£\s?[0-9,]+/gi, '')
            .replace(/placement[^£]{0,80}£\s?[0-9,]+/gi, '')
            .replace(/study abroad[^£]{0,80}£\s?[0-9,]+/gi, '')
            .replace(/year in industry[^£]{0,80}£\s?[0-9,]+/gi, ''); // Specific to Exeter UG
        ...
        if (homeFee && intlFee) {
            if (DEBUG) console.log(`[DEBUG] Exeter: Extracted via directional regex -> Home: £${homeFee}, Intl: £${intlFee}`);
            return { homeFee, internationalFee: intlFee };
        }

        // --- 4. FALLBACK TO GENERIC PARSER ---
        if (DEBUG) console.log(`[DEBUG] Exeter: Missing fees, falling back to generic parser.`);
